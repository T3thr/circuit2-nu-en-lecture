...
ภารกิจหลักของคุณคือ: "สร้างชุดเฉลยและสื่อการสอนใหม่ใน problems/problem-4/solution3/ สำหรับโจทย์ข้อที่ 4 (วงข่ายความนำไฟฟ้า Conductance Network & แหล่งกำเนิดกระแสไม่อิสระ Dependent Source $\alpha i_x$) ที่สอนนิสิตไร้พื้นฐานให้เข้าใจลึกซึ้ง ไร้จุดสะดุด เรียงลำดับความคิดอย่างร้อยเรียงสมบูรณ์แบบ จนสามารถตอบคำถามกรรมการสอบปากเปล่าได้คะแนนเต็ม 100/100!"
...
3. รายละเอียดวงจร:
   ...
   - แหล่งกำเนิดกระแสอิสระ $I_0 [\text{A}]$ (Independent Current Source ป้อนจาก $e$ เข้าปม $a$)
   - แหล่งกำเนิดกระแสไม่อิสระ $\alpha i_x [\text{A}]$ (Current-Controlled Current Source - CCCS ไหลจากปม $b$ เข้าปม $a$)
   - กระแสควบคุม $i_x$: ไหลผ่าน $G_1$ จากปม $a$ ลงสู่ปมอ้างอิง $e$ ($i_x = G_1 V_a$)
   - แหล่งกำเนิดกระแสไม่อิสระแทนค่าได้เป็น $\alpha i_x = \alpha G_1 V_a$
   - ปมไฟฟ้า $a, b$ และปมอ้างอิงลงดิน $e$ ($V_e = 0\text{ V}$)
...
1. บทที่ 1: มโนทัศน์กายภาพความนำไฟฟ้าและแหล่งกำเนิดไม่อิสระ (Physical Intuition & Dependent Sources):
   - ปูพื้นฐานจาก 0: มโนทัศน์ความนำไฟฟ้า $G [\mho]$ กับท่อน้ำ และหลักการทำงานของแหล่งกำเนิดกระแสไม่อิสระ (Dependent Current Source) เปรียบเหมือน "วาล์วเร่งน้ำตามสัดส่วนการไหลของน้ำอีกเส้น"
   - อธิบายมโนทัศน์ทฤษฎีกราฟ (Graph Theory), กิ่งต้นไม้ (Tree Branches), กิ่งร่วม (Links) และการจัดการกับแหล่งกำเนิดไม่อิสระในเมทริกซ์ชุดตัด

2. บทที่ 2: พิสูจน์ KCL, สมการกระแสควบคุม และสมการชุดตัดทีละบรรทัด (Zero Mathematical Gaps):
   - ห้ามใช้คำว่า "ในทำนองเดียวกัน" หรือข้ามขั้นตอนย้ายข้างสมการเด็ดขาด!
   - พิสูจน์สมการ KCL ที่ปม $a$:
     $-I_0 + G_1 V_a + G_2 (V_a - V_b) - \alpha G_1 V_a = 0 \implies (G_1(1-\alpha) + G_2) V_a - G_2 V_b = I_0$
   - พิสูจน์สมการ KCL ที่ปม $b$:
     $G_2 (V_b - V_a) + G_3 V_b + \alpha G_1 V_a = 0 \implies (-G_2 + \alpha G_1) V_a + (G_2 + G_3) V_b = 0$
   - แสดงการย้ายข้างหาดีเทอร์มิแนนต์ $\Delta = G_1 G_2 + G_1 G_3 (1-\alpha) + G_2 G_3$ และพิสูจน์คำตอบ $V_a, V_b$ แบบบรรทัดต่อบรรทัด:
     $$V_a = \frac{I_0 (G_2 + G_3)}{G_1 G_2 + G_1 G_3 (1-\alpha) + G_2 G_3}$$
     $$V_b = \frac{I_0 (G_2 - \alpha G_1)}{G_1 G_2 + G_1 G_3 (1-\alpha) + G_2 G_3}$$
...
5. บทที่ 5: คัมภีร์ซ้อมตอบสอบปากเปล่าคะแนนเต็ม 100/100 (Master Oral Defense Guide 15 ข้อ):
   - เก็งคำถามสอบปากเปล่าเจาะลึก 15 ข้อ สำหรับโจทย์ข้อที่ 4 (เช่น แหล่งกำเนิดไม่อิสระทำให้เมทริกซ์ความนำไม่สมมาตรจริงไหม, ถ้า $\alpha \to 1$ จะเกิดอะไรขึ้นกับแรงดันปม, กระแส $i_x$ มีทิศทางสวนทางจะเกิดอะไรขึ้น)
   - แบ่งโครงสร้างการตอบเป็น 2 ระดับในทุกข้อ:
     [1] บทตอบรอดชีวิต (Defensive Script): ตอบตรงประเด็น ไม่ตกกับดัก (ได้คะแนนผ่าน)
     [2] บทตอบเกียรตินิยม (Proactive Distinction Script): ตอบเชื่อมโยง Graph Theory, Transconductance, Active Feedback Control เพื่อให้กรรมการต้องแจก 100/100
...
2. โครงสร้างแท็บเนื้อหา 6 แท็บ:
   ...
   - แท็บ 2: ปูพื้นฐานความนำไฟฟ้า & แหล่งกำเนิดไม่อิสระ
   - แท็บ 3: พิสูจน์ KCL & สมการกระแสควบคุมทีละบรรทัด
   ...
   - แท็บ 6: Dependent Source & Conductance Lab (ปรับค่า $G_1..G_3, \alpha, I_0$ โต้ตอบ Real-time พร้อมแสดงผลแรงดันปม $V_a, V_b$ และกราฟตอบสนอง)
